...
<body>
    ...
    <button class="calendarButton" onclick="fetchMusicDetailsAndAddToCart()">أضف إلى العربة</button>
    ...
    <script>
        ...
        function fetchMusicDetails() {
            ...
            fetch(`http://localhost/idMusic.php?id=${musicId}`)
                .then(response => response.json())
                .then(data => {
                    console.log('Music details:', data);
                    displayMusicDetails(data);
                    fetchVendorPhoneNumber(musicId);
                    fetchReservedDates(musicId);
                    fetchMap(musicId);
                })
        ...
        function displayMusicDetails(data) {
            ...
            // Create elements for music details
        ...
        function fetchVendorPhoneNumber(musicId) {
            fetch(`get_vendor.php?id=${musicId}`) // Fetch vendor's phone number based on service ID
        ...
        ///////////////////////////////////////map
        function fetchMap(musicId) {
            fetch(`idMusic.php?id=${musicId}`)
                .then(response => response.json())
                .then(data => {
        ...
        function fetchReservedDates(musicId) {
            fetch(`http://localhost/get_reservations.php?service_id=${musicId}`)
                .then(response => response.json())
                .then(reservedDates => {
        ...
        function fetchMusicDetailsAndAddToCart() {
            const serviceId = getMusicIdFromUrl();
            fetch(`http://localhost/idMusic.php?id=${serviceId}`)
                .then(response => {
                    if (!response.ok) {
        ...
        function fetchComments(musicId, musicDetails) {
            fetch(`http://localhost/get_review.php?id=${musicId}`)
                .then(response => response.json())
                .then(reviewData => {
        ...
        function submitComment(event) {
            ...
            const serviceId = getMusicIdFromUrl(); // Get the service ID
        ...
        function fetchReview(musicId, musicDetails, article) {
            fetch(`http://localhost/get_review.php?id=${musicId}`)
                .then(response => response.json())
                .then(reviewData => {
                    // Check if any reviews are returned
