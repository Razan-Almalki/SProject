...
<header>
  ...
  <!-- end header inner -->
    ...
    <section class="showItem-panels" id="musicSection">
    </section>
  <!-- Footer Section -->
  <footer>
    <div class="footer__container">
      ...
      <section class="social__media">
        ...
        <script>
            ...
            fetch('http://localhost/get_music_data.php')
                .then(response => response.json())
                .then(data => {
                    const musicSection = document.getElementById('musicSection');
    
                    data.forEach(music => {
                        // Check if the service type is 'Music'
                        if (music.type === 'Music') {
                            const article = document.createElement('article');
                            article.classList.add('music');
    
                            // Create an image element
                            const img = document.createElement('img');
                            img.src = music.pic ? 'data:image/' + music.pic_type + ';base64,' + music.pic : 'placeholder.jpeg'; // Placeholder image if pic is undefined
                            img.alt = music.name;
    
                            const h2 = document.createElement('h2');
                            h2.textContent = music.name;
    
                            const p2 = document.createElement('p');
                            p2.textContent = 'السعر: ' + music.price + ' ريال';
    
                            const p3 = document.createElement('p');
                            p3.textContent = 'العربون: ' + music.deposit + ' ريال';
                            ...
                            fetchReview(music.id, music, article);
    
                            // Append article to the music section
                ...
                fetch(`http://localhost/get_review.php?id=${musicId}`)
                    ...
                    .then(reviewData => {
                        ...
                        if (reviewData.length > 0) {
                            ...
                            const readMoreButton = createReadMoreButton(musicId);
            ...
            function createReadMoreButton(musicId) {
    
                const readMoreButton = document.createElement('button');
                readMoreButton.textContent = 'إقرأ المزيد';
                readMoreButton.addEventListener('click', () => {
                    window.location.href = `http://localhost/music_details.php?id=${musicId}`;
                });
